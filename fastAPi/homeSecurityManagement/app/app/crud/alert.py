...
def create_alert(db: Session, alert: schemas.SecurityAlertCreate, device_id: int) -> models.SecurityAlert:
    db_alert = models.SecurityAlert(**alert.dict(), device_id=device_id)
    db.add(db_alert)
    db.commit()
    db.refresh(db_alert)
    return db_alert
